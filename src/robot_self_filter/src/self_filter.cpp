...
namespace robot_self_filter
{
  ...
  public:
        ...
        : Node("self_filter")
    {
      ...
      // Publish filtered cloud with RELIABLE QoS so OctoMap and RViz can subscribe
  ...
  private:
    void cloudCallback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud)
    {
      ...
      {
        RCLCPP_WARN(this->get_logger(), "Self-filter failed: %s", e.what());
        return;
      }

      // Restamp with current time so downstream TF lookups succeed
      // (original Gazebo timestamp may be too old for the TF buffer)
      out2.header.stamp = this->now();
      // Keep the original Gazebo frame (ur3e/camera_stand/depth_camera);
      // the static TF bridge handles the coordinate-convention rotation.
      pointCloudPublisher_->publish(out2);

      RCLCPP_INFO(this->get_logger(),
                  "Published filtered cloud: %d → %d points, frame=%s",
                  input_size, output_size, out2.header.frame_id.c_str());
