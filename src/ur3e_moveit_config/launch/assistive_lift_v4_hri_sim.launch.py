"""
Assistive Lift V4 — HRI variant (SIMULATION) with dynamic collision
and human-aware filtering.

Identical FSM to assistive_lift_v4_sim.launch.py (PRE_GRASP, GRASP, LIFT,
AWAIT_INTERACTION, PREP_PLACE, PLACE, RETRACT, MOVE_TO_HOME, ...) but adds:

  /realsense/points
        ├── robot_self_filter        → /cloud_no_robot
        └── human_excluded_cloud_filter (YOLOv8-seg masks out person pixels)
                                     → /cloud_no_human
        └── octomap_gate (blocks during GUIDING, clears OctoMap)
                                     → /cloud_gated
        └── MoveIt OctoMap (sensors_3d.yaml: point_cloud_topic = /cloud_gated)

Plus: loads a custom world (assistive_lift_world_hri.sdf) which embeds a
walking actor so the human-exclusion path can be exercised in sim.

Both OMPL and Pilz pipelines are loaded — Pilz respects the OctoMap at
plan time exactly like OMPL (start state + waypoint collision check).

Untouched:
  - assistive_lift_v4_sim.launch.py
  - assistive_lift_v4_hardware.launch.py
  - ur_simulation_gazebo/worlds/assistive_lift_world.sdf
  - any node either of the above launches uses.
"""
...
def generate_launch_description():
    ...
    # =====================================================================
    # OMPL + Pilz pipelines (shared by move_group and rviz)
    # =====================================================================
    ompl_ns_config = {
    ...
    # --- Ignition Gazebo (HRI world: original assets + walking actor) ---
    hri_world_path = os.path.join(pkg_ur_moveit, "worlds", "assistive_lift_world_hri.sdf")
    ignition = ExecuteProcess(
        cmd=["ign", "gazebo", "-r", hri_world_path],
        output="screen",
    )
    ...
    # --- Robot self-filter (removes robot links from /realsense/points) ---
    robot_self_filter = Node(
        ...
        parameters=[robot_description, {
            ...
            "default_box_padding": [0.15, 0.15, 0.15],
            "default_cylinder_padding": [0.15, 0.15],
            "default_sphere_padding": 0.15,
    ...
    octomap_gate = Node(
        ...
        parameters=[{"use_sim_time": True}],
        remappings=[
            # octomap_gate subscribes to /cloud_filtered → feed it our cleaned cloud
    ...
    # --- Move-to-home (Pilz PTP via /move_to_home/trigger) ---
